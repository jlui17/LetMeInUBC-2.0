...
const TRACKING_TABLE_NAME: string = process.env.TRACKING_TABLE_NAME
  ...
  : "";
const COURSE_INDEX_NAME: string = process.env.COURSE_INDEX_NAME
  ? process.env.COURSE_INDEX_NAME
  : "";
...
export const getTrackingByCourse = async (
  event: GetTrackingByCourseParams
): Promise<any> => {
  ...
  const courseTrackingRestrictedParams = {
    ...
    KeyConditionExpression:
      "courseName = :courseName AND includeRestrictedSeats = :restricted",
  };

  const data = await db
    .query(
      restricted === "true"
        ? courseTrackingRestrictedParams
        : getAllEmailsCourseTrackingParams,
      (error: any, data: any) => {
